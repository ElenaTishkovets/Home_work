 ...
 * 2 Внутри класса мы должны создавать поле хранящее объект класса калькулятор.
 * Для примера можно использовать любой тип калькулятора. Я возьму для примера CalculatorWithMathCopy
 * 3 Для инициализации данного поля требуется использовать конструктор в который будут передавать CalculatorWithMathCopy.
...
public class CalculatorWithCounterAutoAgregation {
    ...
    private CalculatorWithMathCopy calculatorMat;//поле, хранящее объект класса калькулятор

    /**
     * Конструктор для инициализации поля calculatorMat путем передачи объекта на вход конструктора
     */
    public CalculatorWithCounterAutoAgregation(CalculatorWithMathCopy calculatorMat) {
        this.calculatorMat = calculatorMat;
    }
     ...
     */
    public double addAgregation(double a, double b) {
        countOperation++;
        return calculatorMat.add(a, b);
    }
     ...
     */
    public double subtractAgregation(double a, double b) {
        countOperation++;
        return calculatorMat.subtract(a, b);
    }
     ...
     */
    public double multiplyAgregation(double a, double b) {
        countOperation++;
        return calculatorMat.multiply(a, b);
    }
     ...
     */
    public double divideAgregation(double a, double b) {
        countOperation++;
        return calculatorMat.divide(a, b);
    }
     ...
     */
    public double powerAgregation(double base, int exponent) {
        countOperation++;
        return calculatorMat.power(base, exponent);
    }
     ...
     */
    public double absoluteAgregation(double number) {
        countOperation++;
        return calculatorMat.absolute(number);
    }
     ...
     */
    public double squareRootAgregation(double number) {
        countOperation++;
        return calculatorMat.squareRoot(number);
    }
}
